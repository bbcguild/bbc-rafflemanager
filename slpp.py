...
class SLPP:
    ...
    def decode(self, text):
        if not text or type(text) is not str:
            return
        # Note: Currently only removes single-line comments (--), multiline comments not supported
        reg = re.compile('--.*$', re.M)
        text = reg.sub('', text, 0)
        self.text = text
            ...
            while self.next_chr():
                ...
                if self.ch == '\\' and start == end:
                    ...
                    if self.ch != end:
                        s += '\\'
                s += self.ch
        raise ValueError(ERRORS['unexp_end_string'])
            ...
            while self.ch:
                ...
                if self.ch == '{':
                    ...
                    if ch in ('=', ','):
                        ...
                        idx += 1
                        k = None
        raise ValueError(ERRORS['unexp_end_table']) #Bad exit here
