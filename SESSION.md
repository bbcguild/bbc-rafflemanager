...
- Prize follow-up on 2026-04-04: newly created prize cards were pre-filling the ticket field with `0`, which hid the `Ticket #` placeholder and made the field look unlabeled. The admin renderer now treats a zero winner value as empty so fresh cards show the placeholder correctly.
- Public prize-card follow-up on 2026-04-04: split the lower public winner row into three labeled cells in this order: `Ticket #`, `Winner`, `Value`. The layout is being prepared ahead of wiring the new prize value field, so the public card can show a placeholder for `Value` until backend persistence is added.
- Public prize-card follow-up on 2026-04-04: adjust the new three-field public winner row from one shared bar into three separate pills, matching the design direction used elsewhere. Also square off the public card outline and the internal public prize fields so the public page aligns with the newer admin styling.
- Public polish follow-up on 2026-04-04: match the public `Last Updated` line to the admin-style timestamp format (`MM-DD-YY h:mm:ss AM/PM ET`) and dim both public search boxes to the same darker HDR-friendly treatment used on the admin page.
- Public header follow-up on 2026-04-04: replace the clunkier public header search presentation with a compact `Archives` control using emoji arrows for previous/next raffle navigation, while preserving the existing `MAX_HISTORY_DEPTH` archive-limit rules. The old header search markup is now hidden from view so the stats can sit more cleanly to the right.
- Public header follow-up on 2026-04-04: refine the archive placement so the compact `Archives` control lives on the right side of the blue `More Info` bar instead of in the header area. The archive behavior/rules stay the same; only the presentation/location changes.
- Public entrants follow-up on 2026-04-04: show ticket ranges to the public whenever the raffle is `ROLLING`, `COMPLETE/CLOSED`, or being viewed from archive history after a newer raffle has started. The public entrants table renderer is now moving toward a column-config approach so future conditional columns can be added without another structural rewrite.
- Public entrants follow-up on 2026-04-04: the first pass at conditional range-column rendering accidentally referenced `currentRaffleStatus` before that state variable existed, which prevented any entrants from rendering. The page now initializes and updates that status state explicitly before the table renderer relies on it.
- Public entrants follow-up on 2026-04-04: after ranges were restored, the `Total` and `Range` columns were still visually ragged because the header/cell content was not sharing a consistent right-edge anchor. Numeric columns are now right-justified with tabular-number alignment so totals and ranges line up vertically.
- Public entrants follow-up on 2026-04-04: long unbroken entrant names like `AddictedGamer817` could still squeeze the numeric columns because the name track was not constrained enough. The entrants grid now gives `#`, `Total`, and `Range` stable minimum widths while the name column becomes the flexible track, with overflow handling so long names stop pushing numeric columns out of alignment.
- Public entrants follow-up on 2026-04-04: the first “stabilize columns” pass overcorrected and truncated too many normal names. The next adjustment rebalances the whole bottom-row layout to give the entrants table more width than the prize cards, then widens the name track inside the range-enabled table so normal names fit before we rely on clipping.
- Public entrants follow-up on 2026-04-04: move toward a standardized public table design with density presets instead of one layout per special case. Public entrants now choose among `standard`, `range`, `barter`, and `barter-range` modes based on raffle status plus whether any barter values are actually present, while preserving the same overall visual style and avoiding horizontal scroll.
- Public entrants follow-up on 2026-04-04: the first density-mode rollout caused a live `Internal Server Error` because `index.mako` assumed every request had `request.extended_tickets`. The template now falls back safely with `getattr(request, 'extended_tickets', False)` so public rendering does not crash on routes that do not set that flag.
- Public entrants follow-up on 2026-04-04: after recovering from the outage, rebalance the public bottom-row split back to `1fr / 1fr`, compact the public prize cards slightly, and make the entrants table itself denser by mode instead of permanently giving the entrants side extra page width. Keep standard/range modes looser than barter modes so normal weeks stay readable.
- Public entrants follow-up on 2026-04-04: public entrants should no longer rely on a guild-level `extended_tickets` flag to unlock barter columns. The public page now always pulls `json/get/tickets_extended`, and barter mode activates automatically whenever any entrant row has a `Bar` value greater than zero.
- Public entrants follow-up on 2026-04-04: polish the public entrants table by centering the `Total`, `Paid`, `Bar`, and `Range` headers over their numeric columns while keeping the body values right-aligned, and tint `Total` row values to the same warm yellow used by the header `Last Updated` line.
- Admin ticket table follow-up on 2026-04-04: tint the admin `Total` column values to the same warm yellow used by the header `Last Updated` text so the public/admin totals presentation stays visually consistent.
- Admin ticket table follow-up on 2026-04-04: the first color pass accidentally gave the extended-table `Paid` column the same highlight class as `Total`. Fix by reserving the yellow accent strictly for `Total` and giving `Paid`/`Bar` an explicit neutral class that matches `Range`.
- Admin ticket table follow-up on 2026-04-04: add a non-editable totals footer beneath the ticket Handsontable instead of trying to inject a fake Handsontable row. The footer mirrors the table columns visually (`Totals` plus `Total`/`Paid`/`Bar` values) without polluting saved ticket data or interfering with the spare entry row.
- Admin ticket table follow-up on 2026-04-04: the first totals-footer pass was visually correct but the summed values could truncate in narrow numeric columns. Tighten the footer typography and horizontal padding so the totals fit inside the existing admin ticket-table footprint.
- Admin ticket table follow-up on 2026-04-04: replace the faux aligned totals row with a small summary box beneath the ticket table (`Total Tickets`, `Paid`, `Barter`). This is cleaner, avoids truncation/alignment issues, and stays fully separate from Handsontable’s editable grid.
- Domain routing follow-up on 2026-04-04: plan custom-host behavior around one Fly app. `raffles.bbcguild.com` is the canonical public host, `raffle.bbcguild.com` and `tickets.bbcguild.com` redirect there, and `raffle-admin.bbcguild.com` is the canonical admin host that goes straight to the shared `/login` flow. Admin renderer auto-switching is now restricted to the admin host (plus Fly/local fallback), so the public host stays public even for logged-in admins.
