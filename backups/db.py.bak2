...
def resolve_raffle_lookup_code(cur, guild_id, raffle_code):
    ...
    # Exact match always wins when a specific raffle code is provided.
    ...
    # If the user supplied a suffix explicitly and it doesn't exist, stop there.
    if re.search(r"[A-Za-z]", code):
        return None

    # Otherwise, treat the input as a base raffle code and prefer the furthest
    # alphabetical suffix (2611c beats 2611b beats 2611a beats 2611).
    cur.execute("SELECT raffle_guild_num FROM raffles WHERE raffle_guild=? AND raffle_guild_num LIKE ?", (guild_id, code + "%"))
    rows = cur.fetchall()
    if not rows:
        return None

    pattern = re.compile(r"^" + re.escape(code) + r"([A-Za-z]+)?$")
    matches = []

    for row in rows:
        raffle_num = str(row["raffle_guild_num"])
        m = pattern.fullmatch(raffle_num)
        if not m:
            continue

        suffix = (m.group(1) or "").lower()
        matches.append((suffix, raffle_num))

    if not matches:
        return None

    matches.sort(key=lambda item: (item[0] != "", item[0]))
    return matches[-1][1]
